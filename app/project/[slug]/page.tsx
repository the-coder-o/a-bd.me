      ...
      <div key={project?.title} className={'pb-24 pt-[9rem] max-md:pt-[8rem]'}>
        <div className={'flex flex-col gap-14'}>
          <div className={'flex flex-col gap-3 container'}>
            ...
            <motion.h1 initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 2 * 0.1 }} className={'text-3xl font-bold leading-tight tracking-tight text-primary'}>
              {project?.title}
            </motion.h1>
            <motion.p initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 3 * 0.1 }} className={'text-lg leading-tight dark:text-[#b4b4b4] text-[#646464] md:text-xl'}>
              {project?.description}
            </motion.p>
          </div>
          ...
          <div className={'container flex flex-col gap-20'}>
            <div className={'flex flex-col gap-6'}>
              <h2>Tags</h2>
              <div className={'flex flex-wrap gap-3 '}>{project?.tags?.map((tag) => <div className={'whitespace-nowrap rounded-lg bg-[#e8e8e8] dark:bg-[#2a2a2a] dark:text-[#b4b4b4] text-[#646464] px-4 py-1.5 text-sm'}>{tag}</div>)}</div>
            </div>
            <div className="flex flex-col gap-6">
              <h2>Contact</h2>
