...
function FlipNumber({ children }: FlipNumberProps) {
  ...
  const [keyframe, setKeyframe] = useState<'initial' | 'moveDown' | 'out'>('initial')
  ...
  return <span className={clsx('inline-flex', keyframe === 'out' && 'opacity-0 -translate-y-3 duration-75 ease-in-out', keyframe === 'initial' && 'opacity-100 translate-y-0 duration-75 ease-in-out', keyframe === 'moveDown' && 'opacity-0 translate-y-3')}>{numberToShow}</span>
}
